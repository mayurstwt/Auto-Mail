...
function dbToProfile(db: any): Profile {
  return {
    ...
    resumeMimeType: db.resume_mime_type || '',
    resumeData: db.resume_data || '',
    createdAt: new Date(db.created_at).getTime(),
    updatedAt: new Date(db.updated_at).getTime(),
  };
}

function dbToHistoryEntry(db: any): HistoryEntry {
  return {
    id: db.id,
    sentAt: new Date(db.sent_at).getTime(),
    email: db.email,
    profileName: db.profile_name,
    resumeName: db.resume_name,
    subject: db.subject,
    status: db.status as 'sent' | 'failed',
...
function SendTab({ profiles, onSent }: { profiles: Profile[]; onSent: (entries: HistoryEntry[]) => void }) {
  ...
  const sendGroup = async (profile: Profile, groupEmails: string[]): Promise<EmailResult[]> => {
    if (!profile.resumeData) {
      return groupEmails.map(email => ({
        email,
        status: 'failed' as const,
        error: 'Resume content not found for this profile.',
      }));
    }

    // Convert base64 string to Blob
    const binaryString = atob(profile.resumeData);
    const len = binaryString.length;
    const bytes = new Uint8Array(len);
    for (let i = 0; i < len; i++) {
      bytes[i] = binaryString.charCodeAt(i);
    }
    const blob = new Blob([bytes.buffer], { type: profile.resumeMimeType });

    const fd = new FormData();
    fd.append('subject', profile.subject);
    fd.append('body', profile.body);
    fd.append('emails', groupEmails.join('\n'));
    fd.append('resume', blob, profile.resumeName);
...
export default function DashboardClient({ user }: { user: { id: string; email: string; fullName: string } }) {
  ...
  const fetchProfiles = async () => {
    ...
    if (error) {
      console.error('Error fetching profiles:', error.message, '| Details:', error.details, '| Hint:', error.hint);
    } else if (data) {
      setProfiles(data.map(dbToProfile));
    }
  };
  ...
  const fetchHistory = async () => {
    ...
    if (error) {
      console.error('Error fetching history:', error.message, '| Details:', error.details, '| Hint:', error.hint);
    } else if (data) {
      setHistory(data.map(dbToHistoryEntry));
    }
  };
  ...
  useEffect(() => {
    async function loadData() {
      setLoading(true);
      await Promise.all([fetchProfiles(), fetchHistory()]);
      setLoading(false);
    }
    loadData();
  }, []);
  ...
  const handleSent = async (entries: HistoryEntry[]) => {
    ...
    const { error } = await supabase.from('sent_mails').insert(dbEntries);
    if (error) {
      console.error('Error saving sent mails to DB:', error);
    }
    await fetchHistory();
  };
  ...
  const handleSave = async (form: Partial<Profile>, file: File | null) => {
    ...
    try {
      ...
      let resumeName = form.resumeName ?? '';
      let resumeMimeType = form.resumeMimeType ?? '';
      let resumeData = form.resumeData ?? '';
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center flex-col gap-3">
        <IcLoader />
        <span className="text-xs text-[#555]">Loading AutoMail…</span>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Header */}
      <header className="border-b border-[#1a1a1a] px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-3 sticky top-0 z-10 max-w-3xl mx-auto w-full" style={{ background: 'var(--bg)' }}>
        <div className="flex items-center justify-between w-full sm:w-auto gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-semibold text-sm tracking-tight text-white">AutoMail</span>
          </div>

          <div className="flex items-center gap-2 sm:hidden">
            <span className="text-xs text-[#555] truncate max-w-[80px]" title={user.fullName || user.email}>
              {user.fullName || user.email.split('@')[0]}
            </span>
            <button className="btn-ghost text-xs px-2.5 py-1.5" disabled={isPending} onClick={handleLogout}>
        ...
        <div className="flex items-center justify-between w-full sm:w-auto gap-3">
          ...
          <div className="hidden sm:flex items-center gap-3">
            <span className="text-xs text-[#666] truncate max-w-[120px]" title={user.fullName || user.email}>
              {user.fullName || user.email.split('@')[0]}
            </span>
            <button id="logout-btn" className="btn-ghost text-xs px-3 py-1.5" disabled={isPending} onClick={handleLogout}>
              {isPending ? 'Signing out…' : 'Sign Out'}
            </button>
          </div>
        </div>
      ...
      <footer className="text-center text-xs text-[#333] py-4 border-t border-[#111]">
        AutoMail · Emails sent via Gmail SMTP · Resumes and profiles stored in Supabase
      </footer>
